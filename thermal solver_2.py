...
def calculate_convection_coeff(ox, fuel, of_ratio, p_chamber_pa, D_inner, v_gas):
    """
    ...
    I_sp = cea.get_Isp(Pc=pc_psi, MR=of_ratio, eps=40.0, frozen=1, frozenAtThroat=1) # Placeholder for structure
    # Alternatively, get more detailed properties:
    T_k = cea.get_Temperatures(Pc=pc_psi, MR=of_ratio, eps=1.0, frozen=1)[0]
    
    # 2. Get Transport Properties
    # (Cp in kJ/kg-K, Viscosity in millipoise, Pr)
    cp_kj, viscosity_mp, pr, k_g = cea.get_Chamber_Transport(Pc=pc_psi, MR=of_ratio, eps=1.0)
    ...
    R_univ = 8314.46
    M_val = cea.get_IvacCstrTc_ChmMwGam(Pc=pc_psi, MR=of_ratio, eps=1.0)[3]
...
print(f"Calculated convection coefficient at {T_gas - 273.15:.1f} °C: {h_conv:.2f} W/m²K")
T_initial = 25.0 + 273.15  # Initial metal temp (Celsius to Kelvin)

# ==================== TRANSIENT SOLVER ====================
def heat_equation(t, T, r, dr, n_nodes):
...
sol = solve_ivp(heat_equation, [0, t_final], T0, args=(r_nodes, dr, n_nodes), 
