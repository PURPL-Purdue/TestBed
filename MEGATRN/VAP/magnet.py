...
for i in range(1, steps):
    ...
    gradmin = ( calcB(x - travel + 0.0001) - calcB(x - travel) ) / 0.0001
    Imin = F / mur / n / gradmin / (pi * OR**2)

    print(f"OD: {2*OR / in_to_m:.3f}in, mur: {mur:.0f}, F: {F * N_to_lbf:.1f}lbf, I: {1000 * I:.0f}mA, {I**2 * ohmperm * 2*pi*OR*n:.0f}W, Imin: {1000 * Imin:.0f}mA, %diff = {100 * (1 - Imin / I):.0f}")
