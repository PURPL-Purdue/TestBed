function [vonMises,sigma_long, sigma_circ, sigma_rad] = Structures(T_wl, T_wg, wallThicknesses, channelWidth, chamberPressure, chamberDiameter, deltaP)
%Note: Function returns [Qdot, -1, -1] if a wall thickness could not be
%found that meets the strength requirements
%% needs T_wl, T_wg, wallthicknesses, chamberPressure, chamberDiameter, deltaP, channelWidths
%% output: arra of vonmises stresses, tangential, and longitudinal, fail yes or no (can be outside)
...
T_wl = -(wallThicknesses .* flux(heightStepNumber) ./ thermalConductivity - targetTemp); %Calcs prospective coolant wall temperatures ,,flux:

CTE = 0.0000252; % in %change/K
bendMaxs = 3 .* deltaP.* channelWidth .^ 2 ./ (4 .* wallThicknesses.^ 2); %Calcs expected max bending stress in Pa
...
while i <= length(T_wl)
    calcPainArr = [];
    temperatureDist(:) = linspace(targetTemp,T_wl(i),length(wallThicknesses)); %Temperature distribution for integration
    ...
    % 7075
    thermalStress(i) = sum(CTE.* (temperatureDist-293) .* 71700000000)/length(T_wl);
