function [flowTemp,flowVelocity,flowPressure, T_l_reqMatrix, wall_thicknesses, updatedTemps, heightMatrix] = calculateWallTemp2(updatedTemps, heightMatrix, heightArray,T_l_reqMatrix, chamberDiameterArray, wall_thicknesses,channelNum, heightStepArray, flowTempMatrix, flowVelocityMatrix, flowPressureMatrix, width, widthValue, newFluidProperties)
    ...
    T_target = 530; % target gas-side hotwall temp 530 for 7075, 773 for copper
...
% heightStepNumber = 1;
    for(height = heightArray)
        ...
        hInd = find(heightArray == height,1);
        ...
        [temperature2,indexTemp] = min(flowTemp(wInd,:,heightStepNumber)-T_l_reqMatrix(wInd,:,heightStepNumber));
        updatedTemps(wInd,heightStepNumber) = temperature2;
