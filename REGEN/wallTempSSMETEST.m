function [flowTemp,flowVelocity,flowPressure, wall_thicknesses] = wallTempSSMETEST(wall_thicknesses,channelNum, heightStepArray, flowTempMatrix, flowVelocityMatrix, flowPressureMatrix, height, width, heightValue, widthValue, newFluidProperties)
    ...
    % heightStepNumber = 1;
        currentHeightStep = 0.227838/(length(height_steps)-1);
        ...
        [Q_dot, T_wallL, wall_thickness] = HeatFluxSSMETEST(heightStepArray, width, hotWall_dP, k_w, T_target, newFluidProperties);
        if(T_wallL == -1)
            flowTemp(wInd,hInd,heightStepNumber) = -1;
            break
        end
        ...
        mu_wall = py.CoolProp.CoolProp.PropsSI('VISCOSITY','P',pressure,'T',T_wallL,'Water');
